...
def train_batch(input_tensor, target_tensor, model, optimizer,criterion, teacher_forcing_ratio, max_len,tokenizer):
    model.train()
...
def eval_batch(input_tensor, target_tensor, model, criterion, max_len, tokenizer):
    ...
    return loss.item() / len(decoder_output), n_right / prediction.size(0) / prediction.size(1), \
           n_right_sentence / prediction.size(0)