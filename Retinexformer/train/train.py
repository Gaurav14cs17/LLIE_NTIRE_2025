...
def parse_options(is_train=True):
    ...
    parser.add_argument('--opt', type=str, default='Options/RetinexFormer_LOL_v1.yml', help='Path to option YAML file.')
...
def create_train_val_dataloader(opt, logger):  #train loader 和 val loader 一起构建
    ...
    for phase, dataset_opt in opt['datasets'].items():
        if phase == 'train':
            ...
            num_iter_per_epoch = math.ceil(len(train_set) * dataset_enlarge_ratio /(dataset_opt['batch_size_per_gpu'] * opt['world_size']))  #一个epoch遍历一次数据
