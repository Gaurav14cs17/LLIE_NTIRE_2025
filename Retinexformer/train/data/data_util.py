...
from train.data.transforms import mod_crop
from train.utils import img2tensor, scandir
...
def paired_paths_from_lmdb(folders, keys):
    ...
    if not (input_folder.endswith('.lmdb') and gt_folder.endswith('.lmdb')):
        raise ValueError(f'{input_key} folder and {gt_key} folder should both in lmdb 'f'formats. But received {input_key}: {input_folder};'f'{gt_key}: {gt_folder}')
...
def paired_DP_paths_from_folder(folders, keys, filename_tmpl):
    ...
    Returns:
        list[str]: Returned path list.
    """
    assert len(folders) == 3, ('The len of folders should be 3 with [inputL_folder, inputR_folder, gt_folder]. 'f'But got {len(folders)}')
    assert len(keys) == 3, ('The len of keys should be 2 with [inputL_key, inputR_key, gt_key]. 'f'But got {len(keys)}')
    ...
    gt_paths = list(scandir(gt_folder))
    assert len(inputL_paths) == len(inputR_paths) == len(gt_paths), (f'{inputL_key} and {inputR_key} and {gt_key} datasets have different number of images: 'f'{len(inputL_paths)}, {len(inputR_paths)}, {len(gt_paths)}.')
    paths = []
    for idx in range(len(gt_paths)):
        gt_path = gt_paths[idx]
        basename, ext = osp.splitext(osp.basename(gt_path))
        ...
        assert inputL_name in inputL_paths, (f'{inputL_name} is not in 'f'{inputL_key}_paths.')
        ...
        assert inputR_name in inputR_paths, (f'{inputR_name} is not in ' f'{inputR_key}_paths.')
        gt_path = osp.join(gt_folder, gt_path)
