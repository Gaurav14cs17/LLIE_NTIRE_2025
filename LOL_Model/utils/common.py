...
def imgToTensor(img):
    ...
    return torch.from_numpy(img).float().cuda().unsqueeze(0)
def getOut(model,ins):
    bicubic = torch.nn.functional.interpolate(ins,scale_factor=2,mode = 'bicubic',align_corners=False)
    print('bicubic',ins.shape)
    return model(ins,None)
