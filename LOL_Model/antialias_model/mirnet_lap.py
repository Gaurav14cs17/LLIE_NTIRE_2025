...
from antialias_model.antialias import Downsample as downsamp
import sys
...
class MIRNet(nn.Module):
    def __init__(self,cfg=None, in_channels=3, out_channels=3, n_feat=64, kernel_size=3, stride=2, n_RRG=3, n_MSRB=2, height=3, width=2, bias=False):
        super(MIRNet, self).__init__()

        self.conv_in = nn.Conv2d(24, n_feat, kernel_size=kernel_size, padding=(kernel_size - 1) // 2, bias=bias)

        modules_body = [RRG(n_feat, n_MSRB, height, width, stride, bias) for _ in range(n_RRG)]
        self.body = nn.Sequential(*modules_body)

        self.conv_out = nn.Conv2d(n_feat, 12, kernel_size=kernel_size, padding=(kernel_size - 1) // 2, bias=bias)
    ...
    def forward(self, inp_img,inp_s1_img,gt=None):
        ...
        h = self.conv_in(ins)
        h = self.body(h)
        out_dec_level1 = self.conv_out(h)
