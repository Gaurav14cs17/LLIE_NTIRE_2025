...
class MIRNet_v2(nn.Module):
    ...
    def forward(self, inp_img,inp_s1_img,gt=None):
        ...
        out_dec_level1 = self.process(ins)
        ...
        if gt is not None:
            ...
            loss_ic = self.criterion(spy_pred[-1], decomposition_hn[-1]) + self.criterion(spy_pred[-2], decomposition_hn[-2])
...
if __name__ == '__main__':
    ...
    # print("gloabl_net have {:.3f}M paramerters in total".format(sum(x.numel() for x in net.gloabl_net.parameters())/1000000.0))
    # print("layer_7_res have {:.3f}M paramerters in total".format(sum(x.numel() for x in net.layer_7_res.parameters())/1000000.0))
    print("whole have {:.3f}M paramerters in total".format(sum(x.numel() for x in net.parameters())/1000000.0))
    ...
    ins = torch.randn(1, 3, 512, 311).cuda()
    N = 1
    st =time.time()
