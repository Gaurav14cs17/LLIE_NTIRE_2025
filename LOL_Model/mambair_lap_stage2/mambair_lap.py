...
from lrdm_model.pyramid_lap import PyLap
...
class MambaIR(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv_first = nn.Conv2d(24, embed_dim, 3, 1, 1)
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            # for image denoising
            self.conv_last = nn.Conv2d(embed_dim, 12, 3, 1, 1)
    ...
    def forward(self, inp_img,inp_s1_img,gt=None):
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            # for image denoising
            x_first = self.conv_first(ins)
            res = self.conv_after_body(self.forward_features(x_first)) + x_first
            out_dec_level1 = self.conv_last(res)
        ...
        if gt is not None:
            ...
            loss_ic = self.criterion(spy_pred[-1], decomposition_hn[-1]) *5.0 +  self.criterion(spy_pred[-2], decomposition_hn[-2]) + \
                      self.criterion(spy_pred[-3], decomposition_hn[-3]) 
...
if __name__ == '__main__':
    ...
    model = MambaIR(patch_size = 8).cuda()

    print("model have {:.3f}M paramerters in total".format(sum(x.numel() for x in model.parameters())/1000000.0))

    print("filters have {:.3f}M paramerters in total".format(sum(x.numel() for x in filter(lambda p: p.requires_grad,model.parameters())) / 1000000.0))

    
    
    img = torch.Tensor(1, 3, 128, 128).cuda()
    ...
    with torch.no_grad():
        st = time.time()
        for _ in range(N):
            _,high = model(img,img)
    print((time.time() - st)/N,high.shape)